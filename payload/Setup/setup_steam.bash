...
# must be admin user to install:
pkgutil --check-signature /Volumes/steam/Steam.app || exit 2 ;
...
if [[ ( $( codesign --verify --deep --verbose=2 -R="anchor apple generic" --check-notarization /Applications/Steam.app 2>&1 3>&1 | grep -coF "explicit requirement satisfied" 2>/dev/null ; wait ) -gt 0 ) ]] ; then
