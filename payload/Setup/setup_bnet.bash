...
if [[ ( $(mkdir -m 755 ~/Downloads/stage ; wait ; unzip -q ~/Downloads/Battle.net-Setup.zip -d ~/Downloads/stage/ ; wait ; codesign --verify --verbose=2 -R="anchor apple generic" --check-notarization ~/Downloads/stage/"Battle.net-Setup.app" 2>&1 3>&1 | grep -coF "explicit requirement satisfied" 2>/dev/null ; rm -fR ~/Downloads/stage 2>/dev/null ) -gt 0 ) ]] ; then
...
defaults write net.battle "Launch Options/BNA/CONNECTION_STRING_US" -string "us.actual.battle.net" 2>/dev/null || true ;
defaults write net.battle "Launch Options/WoW/LAUNCH_64BIT" -string true 2>/dev/null || true ;
defaults write net.battle "Launch Options/WoW/LOCALE" -string ${LANG:-"en"} 2>/dev/null || true ;
defaults write net.battle "Launch Options/WoW/LOCALE_AUDIO" -string ${LANG:-"en"} 2>/dev/null || true ;
defaults write net.battle "Launch Options/WoW/REGION" -string US 2>/dev/null || true ;
defaults write net.battle "Launcher/Locale" -string ${LANG:-"en"} 2>/dev/null || true ;

if [[ ( $( codesign --verify --verbose=2 -R="anchor apple generic" --check-notarization /Applications/"Battle.net-Setup.app" 2>&1 3>&1 | grep -coF "explicit requirement satisfied" 2>/dev/null ; wait ) -gt 0 ) ]] ; then
